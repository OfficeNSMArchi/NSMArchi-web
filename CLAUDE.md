...
## 2026-05-19 구글맵 블록 완성

### GoogleMap 컴포넌트 (`components/GoogleMap.tsx`)
- `height` prop: 숫자 → 명시적 높이 박스 / 생략 → `position:absolute; inset:0` (부모에 `position:relative` 필요)
- 핀 위치: Bounds 기반 Mercator 보정 계산 (OverlayView 아님)
- 커스텀 스케일바: Web Mercator 공식 `(2π×6378137×cos(lat)) / (256×2^zoom)`, 80px 기준 nice number
- 지도 스타일: roadmap → GRAYSCALE_STYLE + `filter:grayscale(1)`, hybrid → HYBRID_STYLE + `filter:grayscale(1) brightness(1.1)`
- `gestureHandling: "none"` + native wheel 리스너 `{passive:false}` → 지도 위 스크롤 = 줌인/줌아웃만, 페이지 스크롤 없음
- 약관/로고: `disableDefaultUI:true`로 대부분 제거, `.gm-style-cc { opacity: 0.25 }`로 남은 텍스트 흐리게
- 핀 색상: 주황 `#f5761a` + 진한 원 `#c45510`
- `stopPropagation` on click → 카드 닫힘 방지

### MDX 파서 버그 수정 (`lib/mdx.ts`)
- `content` 배열 파싱 시 `map` 타입 케이스 누락 → 지도 블록 데이터 전부 소실되던 버그 수정
- image / map / text 세 케이스 모두 처리

### 환경변수 추가
```
NEXT_PUBLIC_GOOGLE_MAPS_KEY=   # Vercel에도 등록 필요 (없으면 지도 안 뜸)
```
- Google Cloud Console: Maps JavaScript API + Geocoding API 활성화 필요

### 상세 페이지 맵 슬라이드 (`components/project-detail-view.tsx`)
- `slide.type === "map"` → `GoogleMap` 컴포넌트, 너비 `clamp(320px,36vw,620px)`
- 주소 텍스트 하단 오버레이 표시

### 어드민 맵 블록 등록 (`components/admin/ContentBlockEditor.tsx`)
- "+ 맵 블록" 버튼 → 주소 검색 → `/api/geocode` → lat/lng 저장
- `app/api/geocode/route.ts` — 서버사이드 지오코딩 (키 노출 방지, `GOOGLE_MAPS_KEY` 서버 전용)
